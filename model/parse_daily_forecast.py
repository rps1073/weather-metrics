...
def parse_daily_forecast(data):
    ...
    df_formatted['CITY'] =  pd.Series([location_series['name'] for x in range(len(forecast_df.index))])
    df_formatted['STATE'] =  pd.Series([location_series['region'] for x in range(len(forecast_df.index))])
